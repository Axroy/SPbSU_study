...
{
    if (!isEmpty(line))
        insertSortedSingle(table->table[hashFunction(line)], line);
}
...
/*void printLinesWithCollisions(HashTable *table, char fileName[])
{
    ...
    for (int i = 0; i < hashTableSize; i++)
        if (numberOfElements(table->table[i]) > 1)
            fprintf(file, "%s\n", firstElementSymbols(table->table[i]));
...
{
    return hasSuchElement(table->table[hashFunction(line)], line);
}
