...
void addSymbol(Line *line, char symbol, int position)
{
    line->length++;
...
void deleteSymbol(Line *line, int position)
{
    char *newSymbols = new char[line->length];

    for (int i = 0; i < position; i++)
        newSymbols[i] = line->symbols[i];
    for (int i = position + 1; i < line->length; i++)
        newSymbols[i - 1] = line->symbols[i];
